...
export default function GameLobby() {
    ...
    useEffect(() => {
        const initLobby = async () => {
            try {
                ...
                // 1. 并行请求：刷新用户信息 + 获取房间列表
                const [roomsRes] = await Promise.all([
                    api.get('/rooms'),
                    fetchProfile() // Store 里的方法，会更新 currentUser
                ]);
    ...
    return (
            <div className="min-h-screen bg-table-bg text-white pb-20 relative overflow-x-hidden">
                ...
                <header
                    className="sticky top-0 z-40 w-full bg-black/60 backdrop-blur-md border-b border-white/5 px-4 py-3 flex justify-between items-center shadow-lg">
                    {/* User Info */}
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            {!loading && (
                                <div
                                    className="absolute -bottom-1 -right-1 bg-gold-main text-black text-[9px] font-bold px-1.5 py-0.5 rounded-full border border-black">
                                    V{currentUser?.vip_level || 0}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex flex-col gap-1">
                            {/* Chips */}
                            <div
                                className="flex items-center bg-black/40 rounded-full px-2 py-0.5 border border-white/10 gap-2">
                                <div
                                    className="w-4 h-4 rounded-full bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.8)] border border-yellow-300"/>
                                <span className="text-xs font-mono font-bold text-gold-light min-w-[60px]">
                        {loading ? "..." : Number(currentUser?.chips || 0)}
                    </span>
                                <button
                                    className="bg-green-600 hover:bg-green-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px]">
                ...
                <main className="container mx-auto max-w-4xl px-4 py-4 space-y-6">
                            ...
                            <div className="flex w-max space-x-4">
                                {loading ? (
                                    [1, 2].map(i => <Skeleton key={i}
                                                              className="w-[300px] h-[140px] rounded-xl bg-slate-800"/>)
                                ) : (
                                    promotions.map((promo) => (
                                        ...
                                        >
                                            ...
                                            <Badge
                                                className="self-start mb-auto bg-white/20 hover:bg-white/30 backdrop-blur-sm border-none text-white">Event</Badge>
                    ...
                    <Tabs defaultValue="cash" onValueChange={setActiveTab} className="w-full">
                        <TabsList className="w-full bg-slate-900/50 border border-white/5 p-1 h-12">
                            <TabsTrigger value="cash"
                                         className="flex-1 data-[state=active]:bg-gold-main data-[state=active]:text-black font-bold">Cash
                                Game</TabsTrigger>
                            <TabsTrigger value="mtt"
                                         className="flex-1 data-[state=active]:bg-gold-main data-[state=active]:text-black font-bold">Tournament</TabsTrigger>
                            <TabsTrigger value="plo"
                                         className="flex-1 data-[state=active]:bg-gold-main data-[state=active]:text-black font-bold">PLO</TabsTrigger>
                        </TabsList>
                    </Tabs>
                            ...
                            {loading ? (
                                ...
                                rooms.map((room) => (
                                    ...
                                    >
                                        ...
                                        {room.isHot && (
                                            <div
                                                className="absolute top-0 right-0 bg-red-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-bl-lg z-10">
                                                HOT
                                            </div>
                                        )}
                                        ...
                                        <div className="flex justify-between items-center">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <span
                                                        className="text-xs text-slate-500">Min: ${room.minBuyIn}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col items-end">
                                                ...
                                                <Button size="sm"
                                                        className="h-8 bg-table-felt hover:bg-table-felt-dark text-white text-xs px-4">
                                                    Play
                                                </Button>
                                            </div>
                                        </div>
                    ...
                    <section
                        className="bg-gradient-to-r from-slate-900 to-slate-800 rounded-xl p-4 border border-white/5 flex justify-between items-center cursor-pointer hover:border-gold-main/30">
                        <div className="flex items-center gap-4">
                            <div
                                className="w-12 h-12 bg-black/50 rounded-full flex items-center justify-center border border-gold-main/30 text-gold-main">
