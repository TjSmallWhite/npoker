...
export default function PokerRoom() {
    ...
    const {
        ...
        roomConfig, setRoomConfig, fetchProfile // <--- 必须取出这个
    } = useGameStore();
    ...
    useEffect(() => {
        const initRoom = async () => {
            try {
                ...
                // 核心修复 A: 必须先确保拿到最新的个人信息(余额)，否则弹窗会显示余额不足
                // 如果 store 里没有 currentUser (比如刷新后)，就强制拉取
                if (!currentUser) {
                    await fetchProfile();
                }
                ...
                const [infoRes, stateRes] = await Promise.all([
                    api.get(`/rooms/${roomId}`),       // 房间配置
                    api.get(`/rooms/${roomId}/state`)  // 座位状态
                ]);
                ...
                setRoomConfig({
                    name: r.name,
                    smallBlind: Number(r.small_blind || 0),
                ...
                // 3. 填充座位 (解决"看不到坐下按钮"的问题)
                updateGameState({
                    players: stateRes.data.players,
                    // pot: stateRes.data.pot // 如果后端给了 pot
                });

                // 4. 尝试拿手牌 (断线重连)
                if (stateRes.data.room.status === 'playing') {
                    try {
                ...
                router.push('/game'); // 失败踢回大厅
            } finally {
                setIsLoading(false); // 数据全拿到了，再显示界面
            }
        };
    ...
    const handleSeatClick = (seatIndex: number) => {
        // ✅ 修正后：只有当 heroPlayer (我在桌上的角色) 存在时，才拦截
        if (heroPlayer) {
    ...
    if (isLoading) {
        return (
            <div className="w-full h-screen bg-table-bg flex flex-col items-center justify-center text-gold-main gap-4">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-gold-main"></div>
                <div className="font-serif animate-pulse">Entering Room...</div>
            </div>
        );
    };
        ...
        <div className="w-full h-screen bg-table-bg relative overflow-hidden flex flex-col select-none touch-none">
            ...
            <div className="flex-1 relative flex items-center justify-center perspective-[1000px] overflow-hidden">
                <div
                    ...
                    {/* Logo */}
                    <div
                        className="absolute top-[35%] md:top-[25%] font-serif text-table-felt-dark/40 text-2xl md:text-4xl font-bold pointer-events-none">
                        POKER
                    </div>
                        ...
                        <AnimatePresence mode='popLayout'>
                            {communityCards.map((card, i) => (
                                <div key={`${card.code}-${i}`}
                                    ...
                                    <AnimatedCard index={i} rank={card.rank as any} suite={card.suite as any} size="md"
                                                  className="shadow-xl"/>
                                </div>
                            ))}
                        </AnimatePresence>
                                ...
                                {player ? (
                                    <>
                                        ...
                                        <div className="relative z-10">
                                            <Avatar
                                                className={cn("w-10 h-10 md:w-16 md:h-16 border-2 shadow-lg bg-slate-900 transition-all",
                                            ...
                                            {player.isDealer && (
                                                <div
                                                    className="absolute -bottom-1 -right-1 w-3 h-3 md:w-5 md:h-5 bg-white text-black text-[8px] md:text-[10px] font-bold rounded-full flex items-center justify-center border border-slate-400 z-20">D</div>
                                            )}
                                        </div>

                                        {/* 信息条 (手机端极度简化) */}
                                        <div
                                            className="bg-black/80 text-white text-[9px] md:text-xs px-1.5 py-0.5 md:px-3 md:py-1 rounded-full text-center min-w-[50px] md:min-w-[80px] border border-white/10 shadow-lg z-20 -mt-1 backdrop-blur-sm">
                                            <div
                                                className="truncate max-w-[50px] md:max-w-[80px] text-slate-300 leading-none mb-0.5">{player.name}</div>
                                            <div
                                                className="text-gold-main font-mono font-bold leading-none">${player.chips > 9999 ? (player.chips / 1000).toFixed(1) + 'k' : player.chips}</div>
                                        </div>
                                        ...
                                        {player.lastAction && (
                                            <Badge
                                                className="absolute -top-6 z-30 bg-white/90 text-black text-[9px] h-4 px-1 md:text-xs shadow-md animate-in zoom-in">
                                                {player.lastAction}
                                            </Badge>
                                        )}
                                    </>
            ...
            <div
                ...
                {heroPlayer && heroPlayer.cards && (
                    <div
                        className="pointer-events-auto absolute bottom-[80px] md:bottom-[100px] left-1/2 -translate-x-1/2 flex gap-0.5 md:gap-1 z-40 transition-all duration-300">
                ...
                {isMyTurn && (
                    <div
                        className="pointer-events-auto w-full max-w-xl flex items-end gap-2 animate-in slide-in-from-bottom-10 fade-in duration-200 mb-1">
                        <Button
                        ...
                        <div
                            ...
                            <div className="hidden md:block">
                                ...
                                <Slider value={raiseAmount} onValueChange={setRaiseAmount}
                                        min={Number(roomConfig?.bigBlind)} max={heroPlayer?.chips}
                                        step={Number(roomConfig?.bigBlind)} className="mb-2"/>
                            </div>
